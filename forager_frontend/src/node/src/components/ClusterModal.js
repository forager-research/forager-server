...
const ClusterModal = ({
  ...
  const selectedCluster =
    selection.cluster !== undefined && selection.cluster < clusters.length
      ? clusters[selection.cluster]
      : undefined;
  const isSingletonCluster =
    selectedCluster !== undefined && selectedCluster.length === 1;

  let selectedImage;
  if (isSingletonCluster) {
    selectedImage =
      selectedCluster !== undefined ? selectedCluster[0] : undefined;
  } else {
    selectedImage =
      selectedCluster !== undefined &&
      selection.image !== undefined &&
      selection.image < selectedCluster.length
        ? selectedCluster[selection.image]
        : undefined;
  }
  const isClusterView =
    selectedCluster !== undefined && selectedImage === undefined;
  ...
  useEffect(() => {
    const fn = async () => {
      ...
      console.log(endpoints.getAnnotations);
  ...
  const getImageTags = (im) =>
    (annotations[im.id] && annotations[im.id]["tags"]) || [];
  const getImageBoxes = (im) =>
    (annotations[im.id] && annotations[im.id]["boxes"]) || [];
  let selectedTags = [];
  let selectedBoxes = [];
  if (selectedImage !== undefined) {
    ...
    selectedTags = intersectionWith(
      ...selectedCluster.flatMap((im, i) =>
  ...
  const onTagsChanged = async (newTags) => {
    ...
    const deletedNotChanged = differenceWith(
      deleted,
      added,
    ...
    const imageIds =
      selectedImage !== undefined
        ? [selectedImage.id]
        : selectedCluster.flatMap((im, i) =>
            excludedImageIndexes[i] ? [] : [im.id]
          );

    let newAnnotations = { ...annotations };
    for (const id of imageIds) {
      const minusDeleted = differenceWith(
        (annotations[id] && annotations[id]["tags"]) || [],
        deleted,
        isEqual
      );
      let plusAdded = unionWith(minusDeleted, added, isEqual);
      let deduplicated = uniqWith(
        plusAdded.reverse(),
        (a, b) => a.category === b.category
      );
    ...
    let addPromises = added.map(async (t) =>
      addAnnotations(t.category, t.value, imageIds)
    );
    let deletePromises = deletedNotChanged.map(async (t) =>
      addAnnotations(t.category, "TOMBSTONE", imageIds)
    );
    ...
    (e) => {
      ...
      if (isClusterView && key === "ArrowDown") {
        ...
        setSelection({
          ...
          image: Math.min(
            selection.image + 1,
            clusters[selection.cluster].length - 1
          ),
        });
        ...
        if (keyAsNumber >= 1 && keyAsNumber <= BUILT_IN_MODES.length) {
          ...
          const customIndex =
            (keyAsNumber === 0 ? 10 : keyAsNumber) - BUILT_IN_MODES.length - 1;
          boundValue = (customModesByCategory.get(labelCategory) || [])[
            customIndex
          ];
        }
        if (boundValue !== undefined) {
          onTagsChanged([
            ...selectedTags,
            { category: labelCategory, value: boundValue },
          ]);
        } else {
          caught = false;
        }
        ...
        // Deselect all
        setExcludedImageIndexes(
          fromPairs(selectedCluster.map((_, i) => [i, true]))
        );
      } else if (key === "b") {
        toggleShowBoxes();
  ...
  if (selectedCluster !== undefined) {
    header = `${isImageOnly ? "Image" : "Cluster"} ${
      selection.cluster + 1
    } of ${clusters.length}`;
    if (isClusterView) {
      ...
      header += `, image ${selection.image + 1} of ${
        clusters[selection.cluster].length
      }`;
    }
  }
      ...
      {selectedCluster !== undefined && (
        <>
          ...
          <ModalBody>
            <p>
              <b>Key bindings:</b> &nbsp;
              <kbd>&larr;</kbd> <kbd>&rarr;</kbd> to move between{" "}
              {isImageView || isImageOnly ? "images" : "clusters"}
              {isClusterView && (
                <>
                  , <kbd>&darr;</kbd> or{" "}
                  <FontAwesomeIcon icon={faMousePointer} /> to go into image
                  view, <kbd>&uarr;</kbd> to go back to query results,{" "}
                  <kbd>shift</kbd> <FontAwesomeIcon icon={faMousePointer} /> to
                  toggle image selection, <kbd>s</kbd> to select all,{" "}
                  <kbd>i</kbd> to invert selection, <kbd>d</kbd> to deselect all
                </>
              )}
              {isImageView && (
                <>
                  , <kbd>&uarr;</kbd> to go back to cluster view, <kbd>s</kbd>{" "}
                  or <FontAwesomeIcon icon={faMousePointer} /> to toggle image
                  selection
                </>
              )}
              {isSingletonCluster && (
              ...
              (labelCategory ? (
                <p>
                  ...
                  {BUILT_IN_MODES.map(([value], i) => (
                    <>
                      <kbd>{i + 1}</kbd>{" "}
                      <span className={`rbt-token ${value}`}>
                        {labelCategory}
                      </span>{" "}
                    </>
                  ))}
                  {(customModesByCategory.get(labelCategory) || []).map(
                    (name, i) => (
                      <>
                        <kbd>{(BUILT_IN_MODES.length + i + 1) % 10}</kbd>{" "}
                        <span className="rbt-token CUSTOM">
                          {labelCategory} ({name})
                        </span>{" "}
                      </>
                    )
                ...
                <p>
                  <b>Label mode:</b> No category specified
                </p>
              ))}
            <Form>
              <FormGroup className="d-flex flex-row align-items-center mb-2">
                  ...
                  disabled={
                    ...
                    (isClusterView &&
                      selectedCluster.length ===
                        Object.values(excludedImageIndexes).filter(Boolean)
                          .length)
                  }
                  customModesByCategory={customModesByCategory}
                  categoryDispatch={categoryDispatch}
                ...
                {isClusterView ? (
                  <Button
                    color="light"
                    className="ml-2"
                    onClick={() => setSubset(selectedCluster)}
                  >
                  ...
                  <Button
                    color="warning"
                    className="ml-2"
                    onClick={() => findSimilar(selectedImage)}
                  >
                    Find similar images
                  </Button>
                )}
            ...
            {selectedImage !== undefined ? (
              <>
                {isImageView ? (
                  <a
                    href="#"
                    onClick={(e) => toggleImageSelection(selection.image, e)}
                    className="selectable-image"
                  >
                    <div
                      className={
                        "image " +
                        (!!!excludedImageIndexes[selection.image]
                          ? "selected"
                          : "")
                      }
                    >
              ...
              <>
                <div className="mb-1 text-small text-secondary font-weight-normal">
                  ...
                  {selectedCluster.length -
                    Object.values(excludedImageIndexes).filter(Boolean)
                      .length}{" "}
                  of {selectedCluster.length} images (thumbnails:{" "}
                  {imageGridSizes.map((size, i) => (
                    <>
                      <a
                        key={i}
                        href="#"
                        className="text-secondary"
                        onClick={(e) => setImageGridSize(size, e)}
                      >
                        {size.label}
                      </a>
