...
const ValidatePanel = ({
  ...
  const model =
    isValidating && !!validateModel
      ? modelInfo.find((m) => m.name === validateModel.name).with_output
      : undefined;
  ...
  const submitLabels = async (labels) => {
    ...
    if (res.created !== labels.length) {
      console.warn(
        "Incorrect number of annotations created by submitLabels",
        labels,
        res
      );
    }
  ...
  useEffect(() => {
    if (metricsIsLoading)
      getMetrics().finally(() => setMetricsIsLoading(false));
  }, [metricsIsLoading]);

  useEffect(() => {
    if (activeValIsLoading)
      getActiveVal().finally(() => setActiveValIsLoading(false));
  }, [activeValIsLoading]);
  ...
  useEffect(() => {
    if (isValidating) {
      ...
      // on validation stop + once on initial render
      ...
      <div className={isLoading ? "loading" : ""}>
        <div className="d-flex flex-row align-items-center justify-content-between">
          <FeatureInput
            ...
            features={modelInfo.filter(
              (m) => ((m.with_output || {}).pos_tags || []).length > 0
            )}
          ...
          >
            {isValidating ? "Stop" : "Start"} validation
          </Button>
        </div>
        {isValidating && (
          <Row className="text-center mt-3 mb-1">
            <Col md="9" className="d-flex flex-column justify-content-center">
              <Row>
                <Col md="4">
                  <div>
                    <span className="h2 mb-1">
                      {metrics.f1 === null
                        ? "?"
                        : Number(metrics.f1).toFixed(2)}
                    </span>
                    {metrics.f1_std !== null && (
                      <span className="h6">
                        &nbsp;&plusmn; {Number(metrics.f1_std).toFixed(2)}
                      </span>
                    )}
                  </div>
                  <h6>F1 score</h6>
                ...
                <Col md="4">
                  <div>
                    ...
                    {metrics.precision_std !== null && (
                      <span className="h6">
                        &nbsp;&plusmn;{" "}
                        {Number(metrics.precision_std).toFixed(2)}
                      </span>
                    )}
                  </div>
                  <h6>Precision</h6>
                ...
                <Col md="4">
                  <div>
                    ...
                    {metrics.recall_std !== null && (
                      <span className="h6">
                        &nbsp;&plusmn; {Number(metrics.recall_std).toFixed(2)}
                      </span>
                    )}
                  </div>
                  <h6>Recall</h6>
              ...
              <Row className="mt-2">
                {metrics.num_false_positives !== null && (
                  <Col md="4">
                    <div>
                      <span className="h2 mb-1">
                ...
                {metrics.num_false_negatives !== null && (
                  <Col md="4">
                    <div>
                      <span className="h2 mb-1">
